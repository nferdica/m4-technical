...
const InfluencerDetail = () => {
  ...
  useEffect(() => {
    const fetchInfluencer = async () => {
      try {
        const response = await axios.get(`http://localhost:3009/api/influencers/${id}`); // Chamada à API
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:3009/api/influencers/${id}`); // Chamada à API para deletar
  ...
  return (
    <Container maxWidth="sm" sx={{ mt: 4 }}>
      <Paper elevation={3} sx={{ padding: 3 }}>
        ...
        <img src={`http://localhost:3009/uploads/${influencer.photo}`} alt={influencer.name} />
        <Box sx={{ mb: 2 }}>
